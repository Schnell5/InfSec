...
class ClockConfig:
    size = 400
    bg, fg = 'beige', 'brown'
    hh, mh, sh, cog = 'black', 'black', 'blue', 'red'
    picture = None


class PhotoClockConfig:
    ...
...
class Clock(Frame):
    def __init__(self, parent=None, config=ClockConfig):
        Frame.__init__(self, parent)
        self.cfg = config
    ...
    def makeWidgets(self, parent):
        ...
        self.dateLabel = Label(self, bd=3, bg='red', fg='yellow')
        parent.bind('<ButtonPress-1>', self.onSwitchMode)
        parent.bind('<ButtonPress-2>', self.onShowLabel)
    ...
    def onTimer(self):
        ...
        hour, mins, secs = timeTuple[3:6]
        if secs != self.lastSec:
            self.lastSec = secs
            ampm = ((hour >= 12) and 'PM') or 'AM'
            hour = (hour % 12) or 12
            self.display.onUpdate(hour, mins, secs, ampm, self.cfg)
    ...
    def onCountdownExpire(self):
        ...
        msg.config(font=('courier', 80, 'normal'), fg='white', bg='blue')
...
class ClockWindow(Clock):
    def __init__(self, parent=None, config=ClockConfig, name=''):
        Clock.__init__(self, parent, config)
...
if __name__ == '__main__':
    config = ClockConfig()
    myclock = ClockWindow(Tk(), config, 'MyClock')
    myclock.mainloop()
